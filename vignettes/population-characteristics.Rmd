---
title: "Univariate Analysis: CLAHRC NWL Heart Failure Admission Care Bundle"
author: ""
date: "`r Sys.Date()`"
...
```{r popnchars, echo=FALSE, results='asis', cache=TRUE, include=TRUE}
pop_summary <- list(
  "Age" = list(
...
```{r sex-diffdiff-spells, include=TRUE}
...
```{r age-diffdiff-spells, include = TRUE}
...
eth_did_or_XV <- paste(format(exp(coef(model)[1,4]), digits = 3), " (", format(exp(confint(model)[4,1,1]), digits = 3), ", ", format(exp(confint(model)[4,2,1]), digits = 3),")", sep = "")
eth_did_or_XV
eth_did_or_U <- paste(format(exp(coef(model)[2,4]), digits = 3), " (", format(exp(confint(model)[4,1,2]), digits = 3), ", ", format(exp(confint(model)[4,2,2]), digits = 3),")", sep = "")
eth_did_or_U
...
```{r eth-diffdiff-spells, include = TRUE}
...
imd_did_or <- paste(format((bc_exp*coef(bc_model)[4] + 1)^(1/bc_exp), digits = 3), " (", format((bc_exp*confint(bc_model)[4,1]+1)^(1/bc_exp), digits = 3), ", ", format((bc_exp*confint(bc_model)[4,2] + 1)^(1/bc_exp), digits = 3),")", sep = "")
imd_did_or
```

```{r imd-diffdiff-spells, include=TRUE}
...
```{r pop-table, results='asis', message='false', include=TRUE}

patients_first_HF_episode <- patients_first_HF_episode %>% dplyr::mutate(site_period = dplyr::case_when(
...
pop_table <- qwraps2::summary_table(patients_first_HF_episode %>% dplyr::group_by(site_period), pop_summary)
colnames(pop_table) <- stringr::str_remove(colnames(pop_table), "site_period: ")

# Age row 2, Sex row 4, Ethnicity 7,8,10, IMD 13
confints <- rep("", nrow(pop_table))
confints[[2]] <- age_did_or
confints[[4]] <- sex_did_or
confints[[7]] <- eth_did_or_U
confints[[8]] <- eth_did_or_XV
confints[[10]] <- eth_did_or_Y
confints[[13]] <- imd_did_or

pop_table_ci <- cbind(pop_table, "Confidence Interval for Difference in Difference" = confints)
...
```{r age, echo=FALSE, results='asis', fig.height=5, fig.width=7, cache=TRUE, include=TRUE}
...
qwraps2::summary_table(patients_first_HF_episode %>% group_by(site_period),
                       age_summary)
dput(age_summary, file = '../data-out/age_summary.txt')
```

```{r ward, echo=FALSE, results='asis', fig.height=5, fig.width=7, cache=TRUE, include=TRUE}

ward_summary <- with(patients_first_HF_episode, qwraps2::tab_summary(Ward_hf_type))
qwraps2::summary_table(patients_first_HF_episode %>% dplyr::group_by(site_period),
...
```{r age-sp, echo=FALSE, results='asis', fig.height=5, fig.width=7, cache=TRUE, include=TRUE}
...
qwraps2::summary_table(HF_spells %>% dplyr::group_by(site_period),
                       age_summary)
dput(age_summary, file = '../data-out/age_sp_summary.txt')
```

```{r ward-sp, echo=FALSE, results='asis', fig.height=5, fig.width=7, cache=TRUE, include=TRUE}
...
```{r bundles-month, echo=FALSE, results='asis', fig.height=5, fig.width=7, cache=TRUE, include=TRUE}
HF_spells %>% mutate(`Month of Discharge` = floor_date(CSPDischargeTime, 'month')) %>%
  filter(HF.any.code == TRUE) %>%
  group_by(`Month of Discharge`) %>%
  summarize(`Heart Failure as Any Diagnosis Code` = n(), `Heart Failure as Primary Diagnosis` = sum(Heart.Failure.Episode), `Care Bundles Administered` = sum(bundle)) %>% gather(key = "Measure", value = "Count", -`Month of Discharge`) %>%
  ggplot(mapping = aes(x = `Month of Discharge`, y = Count, colour = Measure)) + geom_line() + geom_point() +
  labs(title = 'Number of Completed Heart Failure Spells, and Care Bundles Administered',
    subtitle = 'Northwick Park Hospital, monthly Jan 2012 to July 2017 ', ylab = 'Number of care bundles administered',
    xlab = 'Month of discharge') +
  geom_vline(aes(xintercept = as.POSIXct("2015-06-15 00:00:00", tz = "Europe/London")),
                linetype=4, colour="black")
```

```{r bundles-only-month, echo=FALSE, results='asis', fig.height=5, fig.width=7, cache=TRUE, include=TRUE}
HF_spells %>% mutate(`Month of Discharge` = floor_date(CSPDischargeTime, 'month')) %>%
  filter(HF.any.code == TRUE) %>%
  group_by(`Month of Discharge`) %>%
  summarize(`Heart Failure as Any Diagnosis Code` = n(), `Heart Failure as Primary Diagnosis` = sum(Heart.Failure.Episode), `Care Bundles Administered` = sum(bundle)) %>% gather(key = "Measure", value = "Count", -`Month of Discharge`) %>%
  filter(Measure == "Care Bundles Administered",
         `Month of Discharge` >= as.POSIXct("2015-06-15 00:00:00", tz = "Europe/London")) %>%
  ggplot(mapping = aes(x = `Month of Discharge`, y = Count, colour = Measure)) + geom_line() + geom_point() +
  labs(title = 'Number of Care Bundles Administered per Month',
    subtitle = 'Northwick Park Hospital, monthly June 2015 to July 2017 ', ylab = 'Number of care bundles administered',
    xlab = 'Month of discharge') 
```

```{r bundles-compliance, echo=FALSE, results='asis', fig.height=5, fig.width=7, cache=TRUE, include=TRUE}
...
```{r mortality-chart, echo=FALSE, results='asis', fig.height=4, fig.width=6, cache=TRUE}
HF_spells %>% mutate(yearMonth = floor_date(CSPDischargeTime, 'month')) %>%
  ...
  scale_x_datetime(limits = c(as.POSIXct("2014-04-01 00:00:00", tz = "Europe/London"),
                              as.POSIXct("2017-07-01 00:00:00", tz = "Europe/London"))) +
  geom_vline(aes(xintercept = as.POSIXct("2015-06-15 00:00:00", tz = "Europe/London")),
                linetype=4, colour="black")
```
...
```{r los-chart, echo=FALSE, results='asis', fig.height=4, fig.width=6, cache=TRUE}
units(HF_spells$los) <- 'days'
...
```{r readms-chart-7ac, echo=FALSE, results='asis', fig.height=4, fig.width=6, cache=TRUE}
HF_spells %>% mutate(yearMonth = floor_date(CSPDischargeTime, 'month')) %>%
...
```{r readms-chart-30ac, echo=FALSE, results='asis', fig.height=4, fig.width=6, cache=TRUE}
HF_spells %>% mutate(yearMonth = floor_date(CSPDischargeTime, 'month')) %>%
  ...
  mutate(readmission_rate = numReadm/numDischarged) %>%
  ggplot(mapping = aes(x = yearMonth, y = readmission_rate, colour = StartWardSite)) + geom_line() + geom_point() +
  labs(title = 'Monthly 7 to < 30-day All-Cause Readmission Rate',
    subtitle = 'LNW', ylab = 'Readmission Rate',
    xlab = 'Month of discharge') + scale_y_continuous(limits = c(0,.25)) +
  scale_x_datetime(limits = c(as.POSIXct("2014-04-01 00:00:00", tz = "Europe/London"),
                              as.POSIXct("2017-07-01 00:00:00", tz = "Europe/London"))) +
...
```{r readms-chart-90ac, echo=FALSE, results='asis', fig.height=4, fig.width=6, cache=TRUE}
HF_spells %>% mutate(yearMonth = floor_date(CSPDischargeTime, 'month')) %>%
  ...
  mutate(readmission_rate = numReadm/numDischarged) %>%
  ggplot(mapping = aes(x = yearMonth, y = readmission_rate, colour = StartWardSite)) + geom_line() + geom_point() +
  labs(title = 'Monthly 30 to < 90-day All-Cause Readmission Rate',
    subtitle = 'LNW', ylab = 'Readmission Rate',
    xlab = 'Month of discharge') + scale_y_continuous(limits = c(0,.25)) +
  scale_x_datetime(limits = c(as.POSIXct("2014-04-01 00:00:00", tz = "Europe/London"),
                              as.POSIXct("2017-07-01 00:00:00", tz = "Europe/London"))) +
...
```{r readms-chart-7hf, echo=FALSE, results='asis', fig.height=4, fig.width=6, cache=TRUE, include=TRUE}
HF_spells %>% mutate(yearMonth = floor_date(CSPDischargeTime, 'month')) %>%
  filter(HF.any.code == TRUE, yearMonth >= as.Date("2014-04-01"), yearMonth <= as.Date("2017-06-30")) %>%
  group_by(yearMonth, StartWardSite) %>%
...
```{r readms-chart-30hf, echo=FALSE, results='asis', fig.height=4, fig.width=6, cache=TRUE, include=TRUE}
HF_spells %>% mutate(yearMonth = floor_date(CSPDischargeTime, 'month')) %>%
  filter(HF.any.code == TRUE, yearMonth >= as.Date("2014-04-01"), yearMonth <= as.Date("2017-05-30")) %>%
  group_by(yearMonth, StartWardSite) %>%
  ...
  mutate(readmission_rate = numReadm/numDischarged) %>%
  ggplot(mapping = aes(x = yearMonth, y = readmission_rate, colour = StartWardSite)) + geom_line() + geom_point() +
  labs(title = 'Monthly 7 to < 30-day Heart Failure Specific Readmission Rate',
    subtitle = 'LNW', ylab = 'Readmission Rate',
    xlab = 'Month of discharge') + scale_y_continuous(limits = c(0,0.05)) +
  scale_x_datetime(limits = c(as.POSIXct("2014-04-01 00:00:00", tz = "Europe/London"),
                              as.POSIXct("2017-07-01 00:00:00", tz = "Europe/London"))) +
...
```{r readms-chart-90hf, echo=FALSE, results='asis', fig.height=4, fig.width=6, cache=TRUE, include=TRUE}
HF_spells %>% mutate(yearMonth = floor_date(CSPDischargeTime, 'month')) %>%
  filter(HF.any.code == TRUE, yearMonth >= as.Date("2014-04-01"), yearMonth <= as.Date("2017-03-30")) %>%
  group_by(yearMonth, StartWardSite) %>%
  ...
  mutate(readmission_rate = numReadm/numDischarged) %>%
  ggplot(mapping = aes(x = yearMonth, y = readmission_rate, colour = StartWardSite)) + geom_line() + geom_point() +
  labs(title = 'Monthly 30 to < 90-day Heart Failure Specific Readmission Rate',
    subtitle = 'LNW', ylab = 'Readmission Rate',
    xlab = 'Month of discharge') + scale_y_continuous(limits = c(0,0.05)) +
  scale_x_datetime(limits = c(as.POSIXct("2014-04-01 00:00:00", tz = "Europe/London"),
                              as.POSIXct("2017-07-01 00:00:00", tz = "Europe/London"))) +
...
```{r num_bundles}
HF_spells_included <- HF_spells %>% mutate(`Month of Discharge` = floor_date(CSPDischargeTime, 'month')) %>%
  filter(HF.any.code == TRUE) %>%
  group_by(`Month of Discharge`) %>%
  summarize(`Heart Failure as Any Diagnosis Code` = n(), `Heart Failure as Primary Diagnosis` = sum(Heart.Failure.Episode), `Care Bundles Administered` = sum(bundle)) %>% gather(key = "Measure", value = "Count", -`Month of Discharge`) %>%
  filter(Measure == "Care Bundles Administered",
...
HF_spells_included %>% summarise(total = sum(Count))
