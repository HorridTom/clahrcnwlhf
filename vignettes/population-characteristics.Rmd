...
```{r }
study_dataset <- emergency_spells_reduced %>% filter(!is.na(los)) 
...
```{r }
study_dataset <- study_dataset %>% filter(!is.na(AgeBand))
...
```{r }
study_dataset <- study_dataset %>% filter(Age.est >= 20)
inc_exc <- c(nrow(study_dataset), inc_exc[1] - nrow(study_dataset))
```
After removing those in below the age of 20: `r inc_exc[1]` (removed `r inc_exc[2]`).

```{r }
study_dataset <- study_dataset %>% filter(!((Ward_hf_type %in% c('A&E', 'Ambulatory Care ', 'Community', 'Maternity') | is.na(Ward_hf_type))))
inc_exc <- c(nrow(study_dataset), inc_exc[1] - nrow(study_dataset))
```
After removing those spells at NPH discharged from ward types with no care bundles: `r inc_exc[1]` (removed `r inc_exc[2]`).

```{r }
study_dataset <- study_dataset %>% mutate(disch_day = as.Date(CSPDischargeTime)) %>%
  distinct(PseudoID, disch_day, StartWardSite, .keep_all = TRUE)
inc_exc <- c(nrow(study_dataset), inc_exc[1] - nrow(study_dataset))
```
After removing any duplicates based on PseudoID, StartWardSite and disch_day: `r inc_exc[1]` (removed `r inc_exc[2]`).

```{r }
study_dataset <- study_dataset %>% filter(Sex %in% c('F', 'M'))
inc_exc <- c(nrow(study_dataset), inc_exc[1] - nrow(study_dataset))
```
After removing those with missing gender: `r inc_exc[1]` (removed `r inc_exc[2]`).

```{r }
study_dataset <- study_dataset %>% filter(WardSite %in% c('NPH','EH'))
inc_exc <- c(nrow(study_dataset), inc_exc[1] - nrow(study_dataset))
```
...
```{r }
study_dataset <- study_dataset %>% filter(HF.any.code == TRUE)
inc_exc <- c(nrow(study_dataset), inc_exc[1] - nrow(study_dataset))
```
After removing those without heart failure diagnosis: `r inc_exc[1]` (removed `r inc_exc[2]`).

Test for any duplicated PseudoID & CSPDischargeTime: `r !all(!duplicated(study_dataset %>% select(PseudoID, disch_day)))`.
...
```{r age-diffdiff}
sex_diff_in_diff_data <- patients_first_HF_episode %>% select(StartWardSite, period.date, Sex) %>% mutate(intervention_site = StartWardSite == "NPH", intervention_period = period.date == "B", male = Sex == "M") %>% select(intervention_site, intervention_period, male)
...
HF_bundle_spells <- HF_spells %>% mutate(yearMonth = floor_date(CSPDischargeTime, 'month')) %>% 
  filter(bundle == TRUE, HF.any.code == TRUE) %>%
  left_join(clahrcnwlhf::linked_bundle_data %>% select(-Admission.Datetime), by = "Bundle.Number")
