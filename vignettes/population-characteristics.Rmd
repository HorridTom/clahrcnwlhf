...
pop_table <- qwraps2::summary_table(patients_first_HF_episode %>% group_by(Hospital = StartWardSite, Period = period.date), pop_summary)
...
sp_table <- qwraps2::summary_table(HF_spells %>% group_by(Hospital = StartWardSite, Period = period.date),
                       pop_summary)
...
sp_table_ci <- cbind(sp_table, "Confidence Interval for Difference in Difference" = confints)
...
```{r readms-chart-7hf, echo=FALSE, results='asis', fig.height=5, fig.width=7, cache=TRUE, include=FALSE}
HF_spells %>% mutate(yearMonth = floor_date(CSPDischargeTime, 'month')) %>%
  filter(HF.any.code == TRUE, yearMonth >= as.Date("2014-04-01"), yearMonth <= as.Date("2017-06-30")) %>%
  group_by(yearMonth, StartWardSite) %>%
...
```{r readms-chart-30hf, echo=FALSE, results='asis', fig.height=5, fig.width=7, cache=TRUE, include=FALSE}
HF_spells %>% mutate(yearMonth = floor_date(CSPDischargeTime, 'month')) %>%
  filter(HF.any.code == TRUE, yearMonth >= as.Date("2014-04-01"), yearMonth <= as.Date("2017-05-30")) %>%
  group_by(yearMonth, StartWardSite) %>%
...
```{r readms-chart-90hf, echo=FALSE, results='asis', fig.height=5, fig.width=7, cache=TRUE, include=FALSE}
HF_spells %>% mutate(yearMonth = floor_date(CSPDischargeTime, 'month')) %>%
  filter(HF.any.code == TRUE, yearMonth >= as.Date("2014-04-01"), yearMonth <= as.Date("2017-03-30")) %>%
  group_by(yearMonth, StartWardSite) %>%
