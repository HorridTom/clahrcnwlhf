...
#read30_nc_results_table <- lmer_results_table(hf_read30_all_nc_model_glmer)

#knitr::kable(read30_nc_results_table)
```
...
hf_read30_all_nc_model_glmmTMB <- glmmTMB::glmmTMB(as.integer(allcausereadmission_30) ~
                                                  ...
                                                  month_9 + month_10 + month_11 +
                                                  (1 | pseudoid),
                                                hf_any_data_oc_nc %>% select(-imd_q),
                                                family = poisson(link = "log")#,
                                                #control=glmerControl(optimizer="bobyqa")
                                                )
...
read30_nc_results_table <- lmer_results_table(hf_read30_all_nc_model_glmmTMB)
...
#read90_nc_results_table <- lmer_results_table(hf_read90_all_nc_model_glmer)

#knitr::kable(read90_nc_results_table)
```
...
#confint(hf_read90_all_nc_model_glmmTMB, method = "profile")

read90_nc_results_table <- lmer_results_table(hf_read90_all_nc_model_glmmTMB)
...
\newpage
## logLOS with HF as any diagnosis, no control *********************************************************

```{r loglos-nocont, warning=FALSE, message=FALSE}
...
data_new_ll <- data_new %>%
  mutate(los_dd = log(los_dd))

# Mixed-effects linear model: random effects for pseudoid
hf_los_log_nc_model_lmer <- lmer(los_dd ~ X_t + X_x + X_x_t +
                                       sexM + non_white1 +
                                       ln_age + ward_2 + ward_3 + ward_4 + ward_5 + month_1 + month_2 +
                                       month_3 + month_4 + month_5 + month_6 + month_7 + month_8 +
                                       month_9 + month_10 + month_11 +
...
summary(hf_los_log_nc_model_lmer)


# Display coefficients
knitr::kable(fixef(hf_los_log_nc_model_lmer), col.names = c("coeffs"))

loglos_nc_results_table <- lmer_results_table(hf_los_log_nc_model_lmer)

knitr::kable(loglos_nc_results_table)
```
...
```{r plot-loglos-nc}
# Run model predictions on this averaged data
prediction_output_los_log <- predict(hf_los_log_nc_model_lmer,
                                     newdata = data_new %>% select(-los_dd),
                                     type = "response",
...
# and summarise this data by month.
# Create a new column for the actual monthly readmission rate
los_acts_preds <- data_new_ll %>%
  cbind(prediction_output_los) %>%
  group_by(id_time) %>%
  summarise(los_tot = sum(los_dd),
            total_HF = n(),
            pred_out_los = mean(prediction_output_los)) %>%
  mutate(act_los = los_tot/total_HF)

# Plot the chart
los_acts_preds %>% select(id_time, los_tot, act_los, pred_out_los) %>% 
  ggplot2::ggplot(ggplot2::aes(x=id_time)) +
  ggplot2::geom_line(ggplot2::aes(y=exp(pred_out_los)), colour = "blue") +
  ggplot2::geom_point(ggplot2::aes(y=exp(act_los)), colour = "black")
...
```
```{r}
# qqnorm(resid(hf_read7_all_model_glmer))
# qqline(resid(hf_read7_all_model_glmer))

library(DHARMa)
hf_read7_simOut <- simulateResiduals(fittedModel = hf_read7_all_model_glmer,
                                     plot = FALSE)
plot(hf_read7_simOut)
testOutliers(hf_read7_simOut, type = "bootstrap")
```

```{r}
plotResiduals(simulationOutput = hf_read7_simOut, form = hf_any_data_oc$ln_age)
```
#### 7-day readmissions - age not logged

```{r read7-all-age-nl}
# Mixed-effects poisson model: random effects for pseudoid
hf_read7_all_model_glmer_agenl <- glmer(as.integer(allcausereadmission_7) ~ offset(log(total_HF)) + 
                                    X_t + X_x + X_x_t + X_z + X_z_t + X_z_x + X_z_x_t +
                                    sexM + non_white1 +
                                    exp(ln_age) + ward_2 + ward_3 + ward_4 + ward_5 + month_1 + month_2 +
                                    month_3 + month_4 + month_5 + month_6 + month_7 + month_8 +
                                    month_9 + month_10 + month_11 +
                                    (1 | pseudoid),
                                  hf_any_data_oc,
...
summary(hf_read7_all_model_glmer_agenl)

overdisp_fun(hf_read7_all_model_glmer_agenl)

# Display IRRs
knitr::kable(exp(fixef(hf_read7_all_model_glmer_agenl)), col.names = c("IRR"))
```

```{r}
hf_read7_simOut_agenl <- simulateResiduals(fittedModel = hf_read7_all_model_glmer_agenl,
                                     plot = FALSE)
plot(hf_read7_simOut_agenl)
testOutliers(hf_read7_simOut_agenl, type = "bootstrap")
```
...
### Model fit
```{r qq-plot-los-ic}
qqnorm(resid(hf_los_model_lmer))
qqline(resid(hf_los_model_lmer))
```
```{r}
plot(hf_los_model_lmer)
```


### Loglos
```{r los-log, warning=FALSE, message=FALSE}
hf_any_data_oc_ll <- hf_any_data_oc %>%
  mutate(los_dd = log(los_dd))
...
# Mixed-effects linear model: random effects for pseudoid
hf_los_log_model_lmer <- lmer(los_dd ~ 
                            X_t + X_x + X_x_t + X_z + X_z_t + X_z_x + X_z_x_t +
                            sexM + non_white1 +
                            ln_age + ward_2 + ward_3 + ward_4 + ward_5 + month_1 + month_2 +
                            month_3 + month_4 + month_5 + month_6 + month_7 + month_8 +
...
summary(hf_los_log_model_lmer)


# Display coefficients
#knitr::kable(fixef(hf_los_log_model_lmer), col.names = c("coeffs"))
knitr::kable(exp(fixef(hf_los_log_model_lmer)), col.names = c("exp_coeffs"))
```

```{r los-log-model-out}
los_log_table <- lmer_results_table(hf_los_log_model_lmer)
knitr::kable(los_log_table)
```
```{r}
(los_log_table %>% filter(coeff == "X_t") %>% pull(Estimate)) * (los_log_table %>% filter(coeff == "X_x_t") %>% pull(Estimate))
```
```{r}
(los_log_table %>% filter(coeff == "X_z_t") %>% pull(Estimate)) * (los_log_table %>% filter(coeff == "X_z_x_t") %>% pull(Estimate))
```

```{r}
(los_log_table %>% filter(coeff == "X_t") %>% pull(Estimate)) * (los_log_table %>% filter(coeff == "X_z_t") %>% pull(Estimate))
```

```{r}
(los_log_table %>% filter(coeff == "X_x_t") %>% pull(Estimate)) * (los_log_table %>% filter(coeff == "X_z_x_t") %>% pull(Estimate))
```
```{r}
(los_log_table %>% filter(coeff == "X_t") %>% pull(Estimate)) * (los_log_table %>% filter(coeff == "X_z_t") %>% pull(Estimate))*(los_log_table %>% filter(coeff == "X_x_t") %>% pull(Estimate)) * (los_log_table %>% filter(coeff == "X_z_x_t") %>% pull(Estimate))
```

```{r}
(los_log_table %>% filter(coeff == "X_t") %>% pull(Estimate)) * (los_log_table %>% filter(coeff == "X_x_t") %>% pull(Estimate))*(los_log_table %>% filter(coeff == "X_z_t") %>% pull(Estimate)) * (los_log_table %>% filter(coeff == "X_z_x_t") %>% pull(Estimate))
```

### Model fit loglos
```{r qq-plot-loglos-ic}
qqnorm(resid(hf_los_log_model_lmer))
qqline(resid(hf_los_log_model_lmer))
```
```{r}
plot(hf_los_log_model_lmer)
```
```{r plot-loglos-ic}
# Run model predictions on the averaged data
prediction_output_los_log <- predict(hf_los_log_model_lmer,
                                     newdata = data_new_ll %>% select(-los_dd),
                                     type = "response",
                                     allow.new.levels = TRUE,
                                     re.form=~0)

prediction_output_los_log <- prediction_output_los_log

# Add the predicted outcomes as a column to the averaged dataset,
# and summarise this data by month.
# Create a new column for the actual monthly los
los_acts_preds_log <- data_new_ll %>%
  cbind(prediction_output_los_log) %>%
  group_by(id_time, X_z) %>%
  summarise(los_tot = sum(los_dd),
            total_HF = n(),
            pred_out_los = mean(prediction_output_los_log)) %>%
  mutate(act_los = los_tot/total_HF)

# loglos
los_acts_preds_log %>% select(id_time, X_z, los_tot, act_los, pred_out_los) %>% 
  ggplot2::ggplot(ggplot2::aes(x=id_time)) +
  ggplot2::geom_line(ggplot2::aes(y=pred_out_los), colour = "blue") +
  ggplot2::geom_point(ggplot2::aes(y=act_los), colour = "black") +
  facet_grid(rows = vars(X_z))
...
outcome_predictions_ic <- read7_acts_preds %>% ungroup() %>% rename (act_rate_7dr = act_rate) %>%
  ...
  left_join(death_acts_preds_wm %>% ungroup() %>% select(-total_HF) %>% rename(act_rate_death = act_rate),
            by = c("id_time" = "id_time", "X_z" = "X_z")) %>%
  left_join(los_acts_preds_log %>% ungroup() %>% select(-total_HF) %>%
              mutate(pred_out_los = exp(pred_out_los), act_los = exp(act_los)),
            by = c("id_time" = "id_time", "X_z" = "X_z"))
...
outcomes_predictions_tidy_2_ic %>% ggplot(aes(x = year_month)) + geom_point(aes(y = actual)) + geom_line(aes(y = prediction)) + facet_grid(rows = vars(measure_type), cols = vars(site), scales = "free_y") +
  ylim(0, NA) +
  theme(panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_vline(xintercept = intervention_month, linetype="dotted") +
