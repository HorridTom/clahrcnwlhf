...
death_nc_model_glm <- glm(as.integer(death) ~ offset(log(total_HF)) + X_t + X_x + X_x_t +
...
death_acts_preds <- data_new %>% cbind(prediction_output_death) %>% group_by(X_t) %>% summarise(death = sum(death), total_HF = n(), pred_out_death = mean(prediction_output_death)) %>% mutate(act_rate = death/total_HF)

death_acts_preds %>% select(X_t, death, act_rate, pred_out_death) %>% 
  ggplot2::ggplot(ggplot2::aes(x=X_t)) + ggplot2::geom_line(ggplot2::aes(y=pred_out_death), colour = "blue") + ggplot2::geom_point(ggplot2::aes(y=act_rate), colour = "black")
```
