...
```{r plot-30read-ic}
# Make a copy of the data
covariates_data_any_oc <- hf_any_data_oc %>% filter(X_t <= 67 - 1)
...
```{r plot-90read-ic}
# Make a copy of the data
covariates_data_any_oc <- hf_any_data_oc %>% filter(X_t <= 67 - 3)
...
# Mixed-effects poisson model: random effects for pseudoid
hf_los_model_lmer <- lmer(los_dd ~ offset(log(total_HF)) + 
                            X_t + X_x + X_x_t + X_z + X_z_t + X_z_x + X_z_x_t +
                            sexM + non_white1 +
                            ln_age + ward_2 + ward_3 + ward_4 + ward_5 + month_1 + month_2 +
                            month_3 + month_4 + month_5 + month_6 + month_7 + month_8 +
...
```{r oin-data-ic, fig.height=8, fig.width=5}
