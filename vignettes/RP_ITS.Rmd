...
data_new <- covariates_data_any_oc_nc %>% 
  mutate(sexM = mean(sexM), non_white1 = mean(non_white1), total_HF = 1, ln_age = mean(ln_age), month_1 = mean(month_1), month_2 = mean(month_2), month_3 = mean(month_3), month_4 = mean(month_4),month_5 = mean(month_5), month_6 = mean(month_6), month_7 = mean(month_7), month_8 = mean(month_8), month_9 = mean(month_9), month_10 = mean(month_10), month_11 = mean(month_11), ward_2 = mean(ward_2), ward_3 = mean(ward_3), ward_4 = mean(ward_4), ward_5 = mean(ward_5))
...
I don't understand why the prediction line is so off for the model including the ward variables. Unless it is to do with the following: ward5 perfectly predicts death = 0.

```{r ward-mortality-table}
ward_death_data <- hf_any_data_oc_nc %>% select(death, starts_with("ward")) %>%
...
knitr::kable(table(ward_death_data$death, ward_death_data$ward, useNA = "always"))
```
...
# Create new dataset with averaged values of all covariates
data_new_nw5 <- covariates_data_any_oc_nc_nw5 %>% 
  mutate(sexM = mean(sexM), non_white1 = mean(non_white1), total_HF = 1, ln_age = mean(ln_age), month_1 = mean(month_1), month_2 = mean(month_2), month_3 = mean(month_3), month_4 = mean(month_4),month_5 = mean(month_5), month_6 = mean(month_6), month_7 = mean(month_7), month_8 = mean(month_8), month_9 = mean(month_9), month_10 = mean(month_10), month_11 = mean(month_11), ward_2 = mean(ward_2), ward_3 = mean(ward_3), ward_4 = mean(ward_4), ward_5 = mean(ward_5))
...
outcome_predictions <- read7_acts_preds %>% rename (act_rate_7dr = act_rate) %>%
  ...
  left_join(read90_acts_preds %>% select(-total_HF) %>% rename(act_rate_90dr = act_rate),
            by = c("id_time" = "id_time")) %>%
  left_join(death_acts_preds %>% select(-total_HF) %>% rename(act_rate_death = act_rate),
            by = c("id_time" = "id_time")) %>%
...
outcomes_predictions_tidy_2 <- outcomes_predictions_tidy_1 %>% mutate(prediction = str_detect(measure, "pred"), measure_type = case_when(
  str_detect(measure, "7d") ~ "7 day readmission rate",
  str_detect(measure, "30d") ~ "30 day readmission rate",
  str_detect(measure, "90d") ~ "90 day readmission rate",
  str_detect(measure, "death") ~ "Mortality rate",
  str_detect(measure, "los") ~ "Length of stay",
)) %>%
  select(-measure) %>%
...
hf_any_data_oc_nc <- hf_its_data %>% filter(wardsite == "NPH", HF_any == 1, ward_5 == 0) %>%
  mutate(sexM = if_else(sex=="M",1,0),
                non_white1 = if_else(non_white=="1",1,0),
                total_HF = 1)
...
outcome_predictions %>% ggplot(aes(x = id_time)) + geom_point(aes(y = actual)) + geom_line(aes(y = prediction)) + facet_grid(rows = vars(measure_type), cols = vars(site), scales = "free")
```
