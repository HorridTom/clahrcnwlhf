...
max_date_cutoff <- as.Date('2017-10-31')
```
...
# Apply date restriction to dataset
# TODO: Refactor as single function
# TODO: Use linked bundle to pull in discharge date (currently restricting on admission date)
emergency_spells <- emergency_spells[which(emergency_spells$DischargeDate <= max_date_cutoff & emergency_spells$DischargeDate >= min_date_cutoff),]

bundle_data_clean <- clahrcnwlhf::bundle_data_clean[which(clahrcnwlhf::bundle_data_clean$Admission.Date.Bundle <= max_date_cutoff & clahrcnwlhf::bundle_data_clean$Admission.Date.Bundle >= min_date_cutoff),]
linked_bundle_data <- clahrcnwlhf::linked_bundle_data[which(clahrcnwlhf::linked_bundle_data$Admission.Date.Bundle <= max_date_cutoff & clahrcnwlhf::linked_bundle_data$Admission.Date.Bundle >= min_date_cutoff),]
linked_nicor_data <- clahrcnwlhf::linked_nicor_data[which(clahrcnwlhf::linked_nicor_data$Date.of.discharge <= max_date_cutoff & clahrcnwlhf::linked_nicor_data$Date.of.discharge >= min_date_cutoff),]

emergency_spells_nph <- emergency_spells[which(emergency_spells[,"StartWardSite"] == "NPH"),]

min_data_date <- min(emergency_spells$DischargeDate)
max_data_date <- max(emergency_spells$DischargeDate)
effective_min_date <- max(min_date_cutoff, min_data_date)
effective_max_date <- min(max_date_cutoff, max_data_date)

```

`r nrow(emergency_spells)` discharges from the three hospitals between `r effective_min_date` and
`r effective_max_date` inclusive.
...
```{r, echo=FALSE, fig.height=5, fig.width=7, cache=TRUE}
n_bundle <- clahrcnwlhf::disch_time_table(bundle_data_clean, discharge = FALSE,
  adm_col = "Admission.Date.Bundle")
bundle_max <- max(n_bundle)
matplot(n_bundle, type = c("b"), pch=20, col = 1, xaxt="n", ylim=c(0, ceiling(bundle_max*.11)*10),
...
```{r, echo=FALSE, fig.height=5, fig.width=7, cache=TRUE}
clahrcnwlhf::plot_linking_venn(linked_bundles = linked_bundle_data, linked_nicor = linked_nicor_data)
```

## Linking primary diagnoses of heart failure, care bundles, and national audit data (NWLH)

```{r, echo=FALSE, fig.height=5, fig.width=7, cache=TRUE}
clahrcnwlhf::plot_linking_venn(episodes = emergency_spells[which(emergency_spells$Heart.Failure.Episode == TRUE),], linked_bundles = linked_bundle_data, linked_nicor = linked_nicor_data)
```
