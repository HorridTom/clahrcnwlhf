...
sigma <- as.data.frame(VarCorr(hf_read7_all_nc_model_glmer_spline))[1, "sdcor"]

# Run model predictions on this averaged data
prediction_output_7d_splines <- predict(hf_read7_all_nc_model_glmer_spline,#hf_read7_all_nc_model_glm_spline,
                                        newdata = data_new, #%>%
                                          #dplyr::select(-allcausereadmission_7),
                                        type = "link",
                                        allow.new.levels = TRUE,
                                        re.form=~0#~(1 | pseudoid)
                                        ) + 0.5*sigma^2
...
# and summarise this data by month.
# Create a new column for the actual monthly readmission rate
read7_acts_preds_splines_raw <- data_new %>%
  cbind(prediction_output_7d_splines)

read7_acts_preds_splines_raw %>%
  group_by(allcausereadmission_7) %>%
  summarise(boxplot = list(setNames(boxplot.stats(prediction_output_7d_splines)$stats,
...
read7_acts_preds_splines <- read7_acts_preds_splines_raw %>% 
  group_by(id_time) %>%
  summarise(allcausereadmission_7 = sum(allcausereadmission_7),
            total_HF = n(),
            pred_out_7d = sum(exp(prediction_output_7d_splines))) %>%
...
```
```{r}
pred_ints <- merTools::predictInterval(hf_read7_all_nc_model_glmer_spline, 
                                       newdata = data_new %>% group_by(X_x) %>% 
                                         mutate(sexM = mean(sexM), non_white1 = mean(non_white1), total_HF = mean(total_HF),
                                                ln_age = mean(ln_age),
                                                month_1 = mean(month_1), month_2 = mean(month_2), month_3 = mean(month_3),
                                                month_4 = mean(month_4),month_5 = mean(month_5), month_6 = mean(month_6),
...
```{r}
pred_ints_exp <- exp(pred_ints)

preds_raw <- data_new %>%
  cbind(pred_ints_exp)

acts_preds <- preds_raw %>% 
  group_by(id_time) %>%
  summarise(allcausereadmission_7 = sum(allcausereadmission_7),
            total_HF = n(),
            pred_out_7d = sum(fit),
            pred_out_7d_upr = sum(upr),
            pred_out_7d_lwr = sum(lwr)
            ) %>%
  mutate(act_rate = allcausereadmission_7/total_HF,
         pred_out_7d = pred_out_7d,#/total_HF,
         pred_out_7d_upr = pred_out_7d_upr,#/total_HF,
         pred_out_7d_lwr = pred_out_7d_lwr,#/total_HF
         )

# Plot the chart
acts_preds %>%
  select(id_time, allcausereadmission_7, act_rate, pred_out_7d, pred_out_7d_upr, pred_out_7d_lwr) %>%
  ggplot2::ggplot(ggplot2::aes(x=id_time)) +
  ggplot2::geom_line(ggplot2::aes(y=pred_out_7d), colour = "blue") +
  #ggplot2::geom_line(ggplot2::aes(y=pred_out_7d_upr), colour = "lightblue") +
  #ggplot2::geom_line(ggplot2::aes(y=pred_out_7d_lwr), colour = "lightblue") +
  ggplot2::geom_point(ggplot2::aes(y=act_rate), colour = "black")
```
