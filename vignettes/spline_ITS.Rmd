...
plot_its <- function(model,
  ...
  sigma <- as.data.frame(VarCorr(model))[1, "sdcor"]
  
  term <- if(add_term) {0.5*sigma^2} else {0}
...
```{r plot-death-wm-oc-spline}
covariates_data_any_oc_wm <- hf_any_data_oc_wm

# Create new dataset with averaged values of all covariates
data_new_wm <- covariates_data_any_oc_wm %>%
  #group_by(X_x) %>%
  mutate(sexM = mean(sexM), non_white1 = mean(non_white1), total_HF = mean(total_HF),
         ln_age = mean(ln_age),
         month_1 = mean(month_1), month_2 = mean(month_2), month_3 = mean(month_3),
...
prediction_output_death_wm <- predict(death_model_glm_wm_spline,
                                      type = "response",
                                      data_new_wm %>% select(-death))

death_acts_preds_wm <- data_new_wm %>%
  cbind(prediction_output_death_wm) %>%
  group_by(id_time, X_z) %>%
  summarise(death = sum(death),
            total_HF = n(),
            pred_out_death = mean(prediction_output_death_wm)) %>%
  mutate(act_rate = death/total_HF)
...
data_new <- hf_any_data_oc_nc %>% group_by(X_x, X_z) %>%
  mutate(sexM = mean(sexM), non_white1 = mean(non_white1), total_HF = mean(total_HF),
         ln_age = mean(ln_age),
         month_1 = mean(month_1), month_2 = mean(month_2), month_3 = mean(month_3),
         month_4 = mean(month_4),month_5 = mean(month_5), month_6 = mean(month_6),
...
prediction_output_los_log <- predict(hf_los_log_model_lmer_spline,
