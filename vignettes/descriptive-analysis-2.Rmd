...
```
```{r, fig.height=5, fig.width=7, echo=TRUE, results='asis', cache=TRUE}
ggplot(data=emergency_spells_nph[which(emergency_spells_nph$period.date == "A"),], aes(x=EthnicGroupComp)) + geom_bar(position=position_dodge(), aes(y = (..count..)/sum(..count..)))
```
```{r, fig.height=5, fig.width=7, echo=TRUE, results='asis', cache=TRUE}
ggplot(data=emergency_spells_nph[which(emergency_spells_nph$period.date == "B"),], aes(x=EthnicGroupComp)) + geom_bar(position=position_dodge(), aes(y = (..count..)/sum(..count..)))
```
